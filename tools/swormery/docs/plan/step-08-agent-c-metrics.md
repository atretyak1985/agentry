...
| Dependencies | Step 05 gate PASS; worktree `/Volumes/Work/swarmery-wt-metrics` (work in `tools/swormery`) |
...
Fresh Claude Code session in `/Volumes/Work/swarmery-wt-metrics/tools/swormery` (worktree, branch
`feat/swormery-metrics`). Needs web search for pricing verification.
...
Context:
Репозиторій Swormery після T1. Гілка feat/swormery-metrics (worktree). Прочитай
swormery-design.md (розділи 1-2), docs/jsonl-format.md (секція про usage),
...
5. Backfill-команда: swormery recost — перерахунок cost_usd для всіх turns
...
заінджещених fixtures. Conventional commits у feat/swormery-metrics. Заповни
...
- [ ] `swormery recost` recomputes all turns idempotently
