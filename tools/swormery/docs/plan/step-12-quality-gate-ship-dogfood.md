...
observe the *next* phase's work (approvals + hooks) inside Swormery itself, capturing
...
1. **Autostart**: `swormery install` → `launchctl kickstart -k gui/$(id -u)/com.swormery.daemon`
...
- Keep Swormery open while running the next real work sessions (ideally the Phase 2
  approvals+hooks planning/build).
- Log every gap in `docs/plan/phase2-backlog.md` in the swormery repo: missing
