...
Fresh Claude Code session in `/Volumes/Work/swarmery/tools/swormery`. Read-only access to
...
Context:
Репозиторій Swormery, дизайн — swormery-design.md (прочитай розділи 1-2).
...
4. Додай у docs/jsonl-format.md мапінг "рядок JSONL → таблиці зі
   swormery-design.md" (sessions / turns / events / file_changes):
