# Swormery Control Plane — Implementation Plan (v2)

> Supersedes `../swormery-implementation-plan.md` (draft of 2026-07-12 14:17).
...
Build **Swormery** — a local control plane for monitoring Claude Code agent sessions:
a single Go daemon + embedded React SPA that parses JSONL transcripts from
`~/.claude/projects/`, stores them in SQLite (`~/.swormery/swormery.db`), and serves
...
- **Location**: `tools/swormery/` inside the swarmery repo (module
  `github.com/atretyak1985/swarmery/tools/swormery`). Owner decision 2026-07-12
  (supersedes the original R1 separate-repo choice): swormery is part of swarmery.
  The marketplace no-build-step rule applies to `plugins/**`; `tools/swormery` has
  its own build and a dedicated CI workflow (`.github/workflows/swormery-ci.yml`).
...
- **Sources of truth**: `../swormery-design.md` (schema + UI), `../swormery-ui-mockup.html`
  (design language), `../swormery-agent-tasks.md` (session decomposition T0–T4).
...
5. `swormery install|uninstall|status` — launchd auto-start on macOS
...
| 3. Parallel wave A+B+C | [06](step-06-agent-a-ingest.md), [07](step-07-agent-b-frontend.md), [08](step-08-agent-c-metrics.md), [09 GATE](step-09-quality-gate-parallel-wave.md) | feat/swormery-ingest, feat/swormery-frontend, feat/swormery-metrics (worktrees) | 1 day parallel (LOW-CONFIDENCE — three concurrent sessions, human review async) |
...
- [ ] `swormery install` → daemon running after `launchctl kickstart`; `uninstall` clean
- [ ] No secrets in committed fixtures (grep gate clean)

## Fixes vs the superseded draft (carried R1–R7 + new F1–F8)

Kept from draft: ~~R1 separate repo~~ (superseded 2026-07-12 by owner decision — lives in `swarmery:tools/swormery`), R2 module `atretyak1985`, R3 mockup as frontend
reference, R4 install task, R5 fixture secret scan, R6 FK migration ordering,
R7 `--port`/`SWORMERY_PORT`. New in v2:

| # | Fix |
|---|---|
| F1 | Draft's bootstrap copied docs from `/Volumes/Work/swarmery/temp_files/` — path does not exist. Corrected to the task dir `/Volumes/Work/swarmery-workspace/swarmery/workspace/working/2026/07/12/swormery-control-plane/docs/` |
...
| Stale/wrong model pricing → wrong $ totals | Med | Med | Web-verified pricing.json; unknown model → NULL + warn; `swormery recost` backfill |
...
4. Executors append a filled Completion Report to the bottom of their step file
   (these live in the swormery repo copy under `docs/plan/` after step 01).
...
- `/Volumes/Work/swarmery-workspace/swarmery/workspace/working/2026/07/12/swormery-control-plane/` — `README.md`, `logs/{sessions,agents}.md`, and `docs/{swormery-design.md, swormery-agent-tasks.md, swormery-mvp-prompt.md, swormery-ui-mockup.html, swormery-implementation-plan.md}` (originally analyzed at `…/swarmery/temp/swormery-control-plane/`, since relocated here)
...
- Environment verified 2026-07-12: Go 1.22.3, Node v24.1.0, 13 dirs in `~/.claude/projects/`, `/Volumes/Work/swarmery/tools/swormery` free, remote `github.com/atretyak1985/*`, `/Volumes/Work/swarmery/temp_files` absent (→ F1)

Phase-ordering alternatives considered: (a) single monolithic session per
`swormery-mvp-prompt.md` — rejected: two known-risky points (format spike, parser)
...
→ integrate, matching `swormery-agent-tasks.md` with fixes F1–F8.
