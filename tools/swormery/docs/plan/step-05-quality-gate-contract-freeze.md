...
Short agent session in `/Volumes/Work/swarmery/tools/swormery` for types generation; human for tag +
worktrees.
...
Context: репозиторій Swormery після T1 (step 04). Схема БД і API-відповіді
...
swormery lives at `tools/swormery` inside it; each wave agent works in
`<worktree>/tools/swormery`):

```bash
cd /Volumes/Work/swarmery
git tag swormery-contract-freeze-v1
git worktree add ../swarmery-wt-ingest   -b feat/swormery-ingest
git worktree add ../swarmery-wt-frontend -b feat/swormery-frontend
git worktree add ../swarmery-wt-metrics  -b feat/swormery-metrics
```
...
- [ ] Tag `swormery-contract-freeze-v1` exists on the freeze commit
- [ ] Three worktrees exist with branches `feat/swormery-ingest`, `feat/swormery-frontend`, `feat/swormery-metrics`
