...
Context:
Репозиторій Swormery. Прочитай swormery-design.md і docs/jsonl-format.md
повністю. Fixtures у testdata/fixtures/. Стек зафіксований: Go 1.22+
(модуль github.com/atretyak1985/swormery), modernc.org/sqlite (без CGO),
...
1. Структура репо: cmd/swormery, internal/{ingest,store,api}, web/, Makefile.
2. store: ВЛАСНИЙ простий migration runner з embedded .sql (НЕ golang-migrate —
   бюджет залежностей). Міграція 0001 — ПОВНА схема зі swormery-design.md §2
   (усі таблиці, включно з майбутніми фазами) + службова
   file_offsets(file_path, byte_offset, inode). agents і skills створюй ДО
   events (events має FK на них). БД у ~/.swormery/swormery.db, PRAGMA WAL.
3. ingest: мінімальний парсер — один .jsonl файл (`swormery ingest <file>`),
...
6. Порт: прапорець --port + env SWORMERY_PORT, дефолт 7777.
...
1. go vet + go test зелені; тест парсера на кожному fixture: кількість
   ...
   з parent_event_id.
2. make build → один бінарник; ./swormery ingest <реальний файл з
   ~/.claude/projects> → ./swormery serve → покажи URL і скільки записів
...
- Post-session human verification:
  ```bash
  cd /Volumes/Work/swarmery/tools/swormery && go vet ./... && go test ./... && make build
  ./swormery ingest ~/.claude/projects/-Volumes-Work-swarmery/<newest>.jsonl
  ./swormery serve   # open http://localhost:7777
  ```
...
- [ ] `--port`/`SWORMERY_PORT` work; backend deps ≤ 3 non-stdlib (`go list -m all`)
...
Records ingested (real file): ~/.claude/projects/-Volumes-Work-swarmery/4fdcbda4-….jsonl (744 KB
  ...
  --port flag and SWORMERY_PORT both exercised. go test: 7 tests green; deps: modernc.org/sqlite only.
