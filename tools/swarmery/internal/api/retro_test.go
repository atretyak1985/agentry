...
// subagent_start events with durations, and error events attributed through
// events.turn_id → turns.agent_name.
func retroAgentsServer(t *testing.T) *httptest.Server {
	t.Helper()
	...
	// tech-lead ×2 in the prev window.
	...
	// Errors joined through turn_id: turn 2/4 fold to tech-lead (2 today),
	// turn 1 is the orchestrator (main, 1 today), turn 5 is prev tech-lead.
	mustExec(`INSERT INTO events (session_id, turn_id, ts, type, tool_name, status, payload, dedup_key) VALUES
		(1, 2, ?, 'tool_call', 'Bash', 'error', '{"result":"boom"}', 'e1'),
		(2, 4, ?, 'error',     NULL,   'error', '{"error":"api"}',   'e2'),
		(1, 1, ?, 'tool_call', 'Read', 'error', '{"result":"nope"}', 'e3'),
		(3, 5, ?, 'tool_call', 'Bash', 'error', '{"result":"old"}',  'e4')`,
...
func TestRetroAgents(t *testing.T) {
	...
	t.Run("main fold is top-level, excluded from agents", func(t *testing.T) {
		...
		if out.Main.Runs != 0 || out.Main.Errors != 1 {
			t.Errorf("main = %+v, want runs 0 errors 1", out.Main)
		}
	...
	t.Run("prev window math", func(t *testing.T) {
		// 7-day range → prev is days -13…-7; day10 sits inside it.
		tl := out.Agents[0]
		if tl.Prev.Runs != 2 || !almostEq(tl.Prev.ErrorRate, 0.5) || tl.Prev.CostUSD != 1.0 {
			t.Errorf("tech-lead prev = %+v, want runs 2 rate 0.5 cost 1.0", tl.Prev)
		}
		dbg := out.Agents[1]
		if dbg.Prev.Runs != 0 || dbg.Prev.ErrorRate != 0 || dbg.Prev.CostUSD != 0 {
...
// retroFrictionServer seeds denied tool calls (Bash covered by an exact rule,
// Read by a paren pattern, WebFetch only by a DISABLED rule), two same-key
// error events, and one resolved + one pending permission request.
func retroFrictionServer(t *testing.T) *httptest.Server {
	...
	// Bash: 3 calls / 2 denied; Read: 2 / 1; WebFetch: 1 / 1; Edit: 1 / 0
	// (never denied → excluded from the board).
	...
	// has_rule: exact tool pattern, Tool(argGlob) pattern, and a disabled rule
	// that must NOT count.
	mustExec(`INSERT INTO approval_rules (tool_pattern, action, enabled, created_at) VALUES
		('Bash',           'approve', 1, ?),
		('Read(/tmp/*)',   'approve', 1, ?),
		('WebFetch(*)',    'approve', 0, ?)`, today, today, today)

	// One resolved (300 s wait) + one pending permission request.
	...
	mustExec(`INSERT INTO permission_requests (session_id, tool_name, request_json, status, requested_at, resolved_at) VALUES
		(1, 'Bash', '{}', 'approved', ?, ?),
		(1, 'Read', '{}', 'pending',  ?, NULL)`, requestedAt, resolvedAt, requestedAt)
...
func TestRetroFriction(t *testing.T) {
	...
	t.Run("denied tools ranked, never-denied excluded", func(t *testing.T) {
		if len(out.DeniedTools) != 3 {
			t.Fatalf("denied_tools = %+v, want 3 rows", out.DeniedTools)
		}
